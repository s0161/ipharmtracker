  ...
  return (
    <div className="mb-3">
      ...
      <div className="flex items-center gap-3 bg-ec-card rounded-xl border border-ec-div px-4 py-2.5">
        ...
        <MiniPill label="Completion" value={`${todayStats.completionRate}%`} color={todayStats.completionRate >= 80 ? '#16a34a' : '#d97706'} />
        <MiniPill label="Overdue" value={overdueTasks.length} color={overdueTasks.length > 0 ? '#dc2626' : '#16a34a'} />
        <MiniPill label="Urgent" value={urgentTasks.length} color={urgentTasks.length > 0 ? '#ea580c' : '#16a34a'} />
      </div>

      {/* Overdue escalations */}
      {overdueTasks.length > 0 && (
        <div className="mt-2 px-4 py-2 rounded-xl border" style={{ backgroundColor: '#fef2f2', borderColor: '#fecaca' }}>
          <div className="text-[10px] font-bold tracking-wide mb-1" style={{ color: '#dc2626' }}>OVERDUE ESCALATIONS</div>
          <div className="flex flex-wrap gap-x-4 gap-y-0.5">
            {overdueTasks.slice(0, 4).map(t => (
              <span key={t.id} className="text-[11px]" style={{ color: '#991b1b' }}>
                {t.taskName || t.title}
...
function ManagerStrip({ tasks, todayStats }) {
  ...
  return (
    <div className="flex items-center gap-3 bg-ec-card rounded-xl border border-ec-div px-4 py-2.5 mb-3">
      ...
      <div className="flex-1" />
      <MiniPill label="Pending" value={pendingCount} color="#2563eb" />
      <MiniPill label="Active" value={inProgressCount} color="#d97706" />
      <MiniPill label="Done" value={todayStats.completed} color="#16a34a" />
    </div>
  )
}
...
function ClinicalStrip({ tasks, todayStats }) {
  ...
  return (
    <div className="flex items-center gap-3 bg-ec-card rounded-xl border px-4 py-2.5 mb-3" style={{ borderColor: '#e9d5ff' }}>
      ...
      <span
        ...
        style={{ backgroundColor: '#fdf4ff', color: '#9333ea', border: '1px solid #e9d5ff' }}
      >
...
function MiniPill({ label, value, color }) {
  return (
    <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg" style={{ backgroundColor: `${color}08` }}>
      ...
      <span className="text-[8px] font-semibold tracking-wide uppercase" style={{ color: '#94a3b8' }}>{label}</span>
    </div>
  )
}
